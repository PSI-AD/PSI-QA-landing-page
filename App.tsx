...
const App: React.FC = () => {
  ...
  return (
    <div className={`${darkMode ? 'dark' : ''}`}>
      <div className={`min-h-screen transition-colors duration-300 font-sans overflow-x-hidden ${
        darkMode ? 'dark bg-[#0F111A] text-white' : 'bg-white text-slate-900'
      }`}>
        
        {/* Header - Stacking Layout for Mobile */}
        <header className={`sticky top-0 z-[100] w-full border-b transition-all duration-500 backdrop-blur-3xl ${
          darkMode ? 'bg-[#0F111A]/90 border-white/5 shadow-2xl shadow-black/40' : 'bg-white/90 border-gray-200 shadow-sm'
        }`}>
          ...
          <div className="max-w-7xl mx-auto px-6 py-4 md:h-24 flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
            ...
            <div className="flex items-center gap-4 group cursor-pointer" onClick={() => { setActiveView('projects'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black italic shadow-2xl transition-all duration-500 transform group-hover:rotate-6 ${
                activeView === 'projects' ? 'bg-[#6161ff] text-white shadow-[#6161ff]/40' : 'bg-[#00c875] text-[#0F111A] shadow-[#00c875]/40'
              }`}>
                QA
              </div>
            ...
            <div className={`flex p-1.5 rounded-[1.5rem] border shadow-inner w-full md:w-auto overflow-x-auto no-scrollbar ${darkMode ? 'bg-white/5 border-white/10' : 'bg-gray-100/80 border-gray-200'}`}>
              <button
                ...
                className={`flex-1 md:flex-none flex items-center justify-center gap-3 px-6 md:px-8 py-3 rounded-2xl transition-all duration-300 font-black text-[10px] md:text-xs uppercase tracking-widest whitespace-nowrap ${
                  activeView === 'projects' 
            ...
            <div className="flex items-center gap-4">
               ...
               <a href={DASHBOARD_URL} className={`px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl hover:brightness-110 active:scale-95 flex items-center justify-center ${
                 activeView === 'projects' 
        ...
        {/* Footer */}
        <footer className={`transition-all duration-500 relative overflow-hidden border-t ${
          darkMode ? 'bg-[#0F111A] border-white/5 text-white' : 'bg-white border-gray-100 text-gray-900'
        }`}>
          ...
          <div className="max-w-7xl mx-auto px-8 py-20 flex flex-col lg:flex-row justify-between items-start gap-16 relative z-10">
             <div className="flex flex-col gap-6">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black italic text-lg border ${
                    darkMode ? 'bg-white/10 text-white border-white/10' : 'bg-gray-900 text-white border-gray-800'
                  }`}>QA</div>
                ...
                <div className="flex gap-4 mt-4">
                   <a href={DASHBOARD_URL} className={`h-12 px-8 border rounded-xl font-black text-[10px] uppercase tracking-[0.2em] transition-all flex items-center justify-center ${
                     darkMode ? 'bg-white/10 hover:bg-white text-white hover:text-[#0F111A] border-white/10' : 'bg-white hover:bg-gray-900 text-gray-900 hover:text-white border-gray-200'
                   }`}>Demo</a>
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 gap-12 lg:gap-20">
                <div className="flex flex-col gap-5">
                  ...
                  {['Strategy Hub', 'Execution Engine', 'Audit Logs', 'ROI Analysis'].map(item => (
                    <a key={item} href="#" className={`text-sm font-bold transition-all hover:translate-x-1 ${
                      darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-[#6161ff]'
                    }`}>
                      {item}
                    </a>
                  ))}
                ...
                <div className="flex flex-col gap-5">
                  ...
                  {['Security', 'WCAG Audits', 'Custom SLAs', 'Performance Hub'].map(i => (
                    <a key={i} className={`text-sm font-bold transition-all hover:translate-x-1 cursor-pointer ${
                      darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-[#00c875]'
                    }`}>{i}</a>
                  ))}
                </div>
                <div className="col-span-2 md:col-span-1 flex flex-col gap-5">
                  ...
                  <div className={`flex items-center gap-3 text-[10px] font-black px-5 py-3 rounded-2xl border shadow-inner ${
                    darkMode ? 'text-[#00c875] bg-[#00c875]/10 border-[#00c875]/20' : 'text-[#00c875] bg-[#00c875]/5 border-[#00c875]/10'
                  }`}>
          ...
          <div className={`max-w-7xl mx-auto py-10 border-t px-8 flex flex-col sm:flex-row justify-between text-[10px] font-black uppercase tracking-widest gap-6 ${
